...
import {id} from './Cadastros.js';

class Home1 extends React.Component {
 static navigationOptions = {
    title: 'Editar/Eliminar Utilizador'
   };
     constructor(props) {
    super(props);
    this.state = {
...
componentWillMounttt = async () =>{
  console.log(id);
try{
  if(this.state.vagas == null){
    Alert.alert(`Digite uma Password`);
  ...
  //console.log(headers);
    ...
    const response=await fetch('http://194.210.91.78/ProjetoEstagios/web/api/users/'+ id, {
      ...
      body:JSON.stringify({
        ...
        password: this.state.vagas,
        email:this.state.email,
 }),
    });
    Alert.alert(`Utilizador Editado`);
    this.setState({  data: ''});
    this.setState({  vagas: ''});
    this.setState({  email: ''});
 ...
 try{
    ...
    //console.log(headers);
      ...
      this.setState({  data: resp.username});
      this.setState({  vagas: JSON.stringify(resp.password)});
   ...
   try{
      ...
      //console.log(headers);
        ...
        Alert.alert(`Utilizador Eliminado`);
  ...
  //  console.log(id);
    return (
      ...
      <Text h4>Editar/Eliminar Utilizador</Text>
